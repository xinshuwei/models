...
class BaseTest(tf.test.TestCase):
  ...
  def data_root(self):
    ...
    return os.path.join(os.path.split(
        os.path.abspath(self.file))[0], "test_data")
  ...
  def _construct_test_case(self, name, graph, ops_to_eval,
...
class GoldenParser(argparse.ArgumentParser):

  def __init__(self):
    super(GoldenParser, self).__init__()
    self.add_argument(
        "--regenerate", "-regen",
        action="store_true",
...
def main(argv, test_class):
  flags = GoldenParser().parse_args(argv[1:])
  if flags.regenerate:
